...
def process_all_images(figs_folder="figs", output_file="results.xlsx"):
    ...
    for i, image_path in enumerate(image_files, 1):
        print(f"\n{'='*80}")
        print(f"处理第 {i}/{len(image_files)} 个图片")
        print(f"{'='*80}")
    ...
    with pd.ExcelWriter(output_file, engine='openpyxl') as writer:
        df.to_excel(writer, sheet_name='All_Curve_Data', index=False)
    ...
    print(f"所有图片处理完成！")
    print(f"共处理 {len(processed_results)} 个图片，生成 {len(all_excel_data)} 行数据")
    print(f"结果已保存到: {output_file}")
    
    # 创建汇总工作表
    create_summary_sheet(processed_results, output_file)
    
    # 显示处理统计信息
    print_processing_summary(processed_results)
...
def print_processing_summary(processed_results):
    """
    打印处理汇总信息
...
def create_summary_sheet(processed_results, output_filename):
    """
    创建汇总信息工作表
