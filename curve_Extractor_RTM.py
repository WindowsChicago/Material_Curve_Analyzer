...
class CurveExtractor:
    ...
    def extract_curve_points_advanced(self, mask, num_points=128):
        ...
        # 步骤0: 初步边界过滤 - 移除明显超出图像边界的点
        points = self.filter_boundary_points(points)
        ...
        # 最终边界检查 - 确保所有点都在图像范围内
        sampled_points = self.filter_boundary_points(sampled_points)
        
        return sampled_points
    
    def filter_boundary_points(self, points, margin=5):
        """过滤掉超出图像边界的点"""
        if len(points) == 0:
            return points
        ...
        # 过滤条件
        valid_mask = (points[:, 0] >= x_min) & (points[:, 0] <= x_max) & \
                     (points[:, 1] >= y_min) & (points[:, 1] <= y_max)
        
        filtered_points = points[valid_mask]
        
        if len(filtered_points) < len(points):
            print(f"边界过滤: 移除了 {len(points) - len(filtered_points)} 个超出边界的点")
    ...
    def filter_extreme_points(self, points, window_size=1, threshold_factor=2.0):
        ...
        if len(points) <= window_size:
            return self.filter_boundary_points(points)
        ...
        for i in range(n):
            # 首先检查边界条件
            if (points[i, 0] < 0 or points[i, 0] >= self.width or 
                points[i, 1] < 0 or points[i, 1] >= self.height):
                continue  # 跳过明显超出边界的点
        ...
        result = np.array(filtered_points) if filtered_points else points
        return self.filter_boundary_points(result)
        ...
        for i in range(1, len(points)):
            ...
            if x_gap > self.width * max_gap_ratio:
                ...
                for j in range(1, num_insert + 1):
                    ...
                    # 检查插值点是否在图像范围内
                    if (0 <= x_insert < self.width and 0 <= y_insert < self.height):
    ...
    def resample_curve(self, points, num_points):
        ...
        try:
            ...
            # 生成新的x坐标
            x_min = max(0, np.min(unique_points[:, 0]))
            x_max = min(self.width-1, np.max(unique_points[:, 0]))
            ...
            x_max = min(self.width-1, np.max(points[:, 0]))
            x_new = np.linspace(x_min, x_max, num_points)
