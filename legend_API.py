...
class LegendDetectionPipeline:
    ...
    def process_image_pipeline(self, image_path, yolo_threshold=0.3):
        """完整的处理流程：YOLO检测 -> 裁剪 -> 图例提取"""
        print("=" * 60)
...
def main():
    ...
    try:
        # 执行完整的处理流程
        results = pipeline.process_image_pipeline(image_path, yolo_threshold=0.3)
