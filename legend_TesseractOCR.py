...
class OCRLegendTextExtractor:
    ...
    def enhance_black_text(self, img):
        """专门增强黑色文本"""
        # 转换为灰度图
    ...
    def detect_black_text_regions(self, img):
        """专门检测黑色文本区域，排除最左边1/15区域的文字"""
        # 增强黑色文本
        text_enhanced = self.enhance_black_text(img)
    ...
    def advanced_ocr_for_black_text(self, img, text_regions):
        """专门为黑色文本优化的OCR"""
        text_data = []
    ...
    def extract_legend(self, image_path):
        ...
        # 检测黑色文本区域（已排除最左边1/15区域）
        text_regions = self.detect_black_text_regions(img)
        
        # 合并靠近的文本区域
        merged_text_regions = self.merge_close_text_regions(text_regions)
        
        # OCR识别
        text_data = self.advanced_ocr_for_black_text(img, merged_text_regions)
...
def main():
    ...
    try:
        ...
        # 可视化结果
        extractor.visualize_results(image_path, "black_text_result.png")
